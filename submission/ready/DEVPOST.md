...
- Live app: https://second-player-lab.asterai.chatgpt.site/
- Source and MIT license: https://github.com/mcpilotandmore/mcpilot-adaptive-play-lab at `v1.0.0`
- Demo video: https://youtu.be/etHmptvGNvc (119 seconds)
...
MCPilot Adaptive Interactive Play Lab turns that guesswork into one short, reversible experiment.
...
### Post-video live validation

This final played pair was completed after the published 1:59 demo and is documented in the repository; the video demonstrates the permission-gated workflow, not these exact trial IDs or this exact outcome.

The same player tested **One-hand-left controls and reduced motion** through **In-app browser WebMCP: inspect, read signals, propose, visible approval, exact apply, then exact undo.**, while preserving **The player still steers, collects signals, and avoids visible hazards on the same timed course.**.

- Baseline: `trial-ebaf831f-a54c-4359-99a8-69253269ad4f`
- Adapted trial: `trial-b6cf2c66-4577-4153-bc9d-d07eb733c6ae`
- Player check-in: **Worse for me**
- Score: 1750 → 1750 (0)
- Accuracy: 100% → 100% (0 pt)
- Collision rate: 0/10s → 0/10s (0/10s)
- Median collect time: 2307 ms → 2970 ms (+663 ms)
- Verdict: **needs another iteration**
- Player observation: The player selected Worse for me through the visible player check-in after the adapted trial.

**Accuracy: 100% to 100% (0 pt). The run did not produce enough qualifying gains to clear the product's success policy, so the page calls for another iteration.**
...
The exact application source is commit `cb7f19545b1c22bb32003435a5655459ea2945c9`, deployed as Sites version 21. Its deterministic behavior is covered by 101 product tests. Gameplay telemetry, settings, approvals, and comparisons remain device-local; there is no account system, backend MCP server, or database. Optional voice streams microphone audio and a bounded page-state summary only while connected, and the standard API key remains server-side.
